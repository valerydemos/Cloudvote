...
var statusId = 'CkLQhz2sbTFrJhSSx';

Template.acTemplate.rendered = function () {

  container = this.find("#container");

  var appstatus = Appstatus.findOne(statusId);
...
Template.body.helpers({
  ...
  currentTemplate() {
    var status = Appstatus.findOne(statusId);
...
Template.controlPanel.helpers({
  appStatus() {
    return Appstatus.findOne(statusId);
  }
})
...
Template.controlPanel.events({
  'click .setAdminMode'() {
    ...
    Appstatus.update(statusId, { $set: { currentTemplate: 'adminPanel' } });
  },
  'click .setSurveyMode'() {
   
    Appstatus.update(statusId, { $set: { currentTemplate: 'surveyPanel' } });
  },
  'click .setDashboardMode'() {
    
    Appstatus.update(statusId, { $set: { currentTemplate: 'dashboardPanel' } });
  },
  'click .startSurvey'() {
    Appstatus.update(statusId, { $set: { currentTemplate: 'surveyPanel', votingEnabled: true, questionIndex: 0 } });
    
    Meteor.call('voting.start', { reset: true });

  },
  'click .stopVoting'() {   
    Appstatus.update(statusId, { $set: { currentTemplate: 'surveyPanel', votingEnabled: false, questionIndex: 0 } });

  },
  'click .nextQuestion'() {
    var appstatus = Appstatus.findOne(statusId);
    Appstatus.update(statusId, { $set: { currentTemplate: 'surveyPanel', votingEnabled: true, questionIndex: appstatus.questionIndex + 1 } });
  },
...
Template.survey.events({
  'click .submitVote'() {
   // Clients.update(localStorage.getItem("clientId") , { $set: { answered: true } })
...
Template.survey.helpers({
  isDeactivated: function () {
    ...
    var answered = Clients.findOne(localStorage.getItem("clientId")).answered;
